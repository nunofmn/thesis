...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.4]{Figures/evaluation/request-rate-reads-linear.pdf}
  \caption{Demanded request rate.}
...
Also, has stable average response times, only increasing when performing an average effective request of~$\approx$ 91 requests/second (100 requests/second demanded request rate) where the average response time is of 2381.225 ms.
...
When the client starts to saturate, the average response time starts to increase to the 700–900 ms range.
...
% TODO: efectuar mais umas runs e verificar se taxa de erros persiste

\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.4]{Figures/evaluation/average-response-time-reads.pdf}
  \caption{Average response time.}
...
\begin{figure}[htb]
  \centering
  \includegraphics[scale=0.4]{Figures/evaluation/error-ratio-reads.pdf}
...
We calculated the cost also for different confirmation speeds in the network.
...
\begin{table}[]
  ...
  \resizebox{\textwidth}{!}{%
    \begin{tabular}{lcc}
      ...
      SafeLow (\textless 20 minutes) & 4 & 20 \\ \hline
      Standard (\textless 5 minutes) & 6 & 5 \\ \hline
      Fast (\textless 2 minutes) & 25 & 2
    \end{tabular}%
  }
...
We defined two different scenarios with variable web of trust size, mainly to analyze the cost of execute recursively the \textit{checkValidity} function in the context of the \textit{unsignEntity} function.
We also initiated entities with different names length, so it is possible to verify the cost difference of increasing the entities' name length.
...
The cost of each operation in function of the transaction confirmation speed is depicted in Figure~\ref{fig:scenario1-operation-cost}.
