...
\begin{figure}
  \centering
  \subfigure[Scenario A]{\label{fig:a}\includegraphics[width=0.4\linewidth]{Figures/web-trust-scenario-1.pdf}}
  \subfigure[Scenario B]{\label{fig:b}\includegraphics[width=0.4\linewidth]{Figures/web-trust-scenario-2.pdf}}
...
This \ac{API} will run on the \ac{CSP} network, and can be deployed in two ways: a single API instance for all the nodes controled by the \ac{CSP}, or a multiple API instances, one for each node, running in the same server.
This \ac{API} will have the classic components usually associated with a RESTful service: every endpoint returns JSON documents and is possible to address every stored resource (certificates, entities, transactions, etc) in the system.
...
An example of such an application that uses the IDChain API is the IDChain Management App, that we will describe in the next subsection.
% TODO: REFERENCE SUB-CHAPTERS
