...
This \ac{API} will run on the \ac{CSP} network, and can be deployed in two ways: a single API instance for all the nodes controled by the \ac{CSP} as shown in Figure~\ref{fig:multi-deploy}, or a multiple API instances, one for each node, running in the same server, shown in Figure~\ref{fig:single-deploy}.
