...
The main design goal to build a fully decentralized DHT-based system that is secure and can be closed to participation in a federated model, while minimizing trust in the intervenients.
...
This thesis is especially focused in building the mechanism to be used in the Global Registry component of the reThink architecture, but we want to decouple the DHT system, in order for it to be possible to use the DHT independently, enable developers to build other DHT-based systems.
...
The classic approach to providing a secure communication channel between nodes is by using the \ac{TLS} protocol coupled with a X509 PKI infrastructure.
...
With the advent of the blockchain technologies and smart contracts, it is possible to build entities and organizations like the DAO~\cite{ralphc.merkle2016}, entities that are truly transparent, that can't be stopped or interrupted, and more importantly can't be corrupted or tampered.
...
As said before, this is aligned with the fact that as we are working in a federated model, we may want for each \ac{SP} cto have multiple nodes, in order for the \ac{DHT} to be scalable.
...
An advantage of building this \ac{API} for accessing the IDChain functionalities, is that allows to build applications and services that easily interact with it through \ac{HTTP} while being language-agnostic, easing the access to the system.
...
It is possible to trigger events in smart contracts that when executed in a transaction context, will notify the blockchain client, i.e. when a client attaches a valid block to the blockchain the client will be notified of this event.
...
Using a blockchain to build a PKI, it is possible to minimize the trust, relatively to a centralized solution, like a CA.
% time ordered?
