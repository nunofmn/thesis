...
Then we also do the same verification as in the client side, and fetch the certificate from the IDChain API, and verify the certificate fingerprint against the one stored in the blockchain, and check if the message sender node identifier is equal to the node identifier stored in the client certificate.
...
We encode the node identifier, IP address and host name in as entries in the X509v3 Subject Alternative Name extension.
