...
In Section~\ref{subsection:dht-mechanism} we explain how we modified the nodes communication process, in order to be possible to integrate our mechanism in the DHT.
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    ...
    \State $revocations \gets empty list$ \Comment{identifier of revoked certificates}
    \State $certificates \gets empty map$ \Comment{map associating a peer identifier to a certificate}
    \State $entities \gets empty map$ \Comment{map associating a Ethereum account address to a entity}
  \end{algorithmic}
\end{algorithm}
...
It is also necessary to allow entities to revoke certificates that have issued.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \If{$entities[entity].signers.length <= MINIMUM \allowbreak \textbf{ and } !previousEntityState \textbf{ and } !entities[entity].boostraper $}
